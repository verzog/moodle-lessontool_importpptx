...
final class importer_test extends \advanced_testcase {
    ...
    public function test_captioned_images_become_card_group_when_enabled(): void {
        ...
        // request-unique id (so cards from different pages never collide when
        // several pages render on one screen).
        $this->assertStringContainsString('local-lessonimportpptx-cardmodal', $out->html);
        preg_match_all('/data-bs-target="#(lessonImportCard\w+)"/', $out->html, $targets);
        preg_match_all('/id="(lessonImportCard\w+)"/', $out->html, $ids);
    ...
    public function test_smartart_slide_kept_as_image_when_enabled(): void {
        ...
        $this->assertStringContainsString('@@PLUGINFILE@@/slide-4.png', $pages[3]->contents);
        $this->assertStringNotContainsString('<ul>', $pages[3]->contents);
        $fs = get_file_storage();
        $this->assertTrue($fs->file_exists($context->id, 'mod_lesson', 'page_contents', $pages[3]->id, '/', 'slide-4.png'));
    ...
    public function test_smartart_slide_stays_editable_by_default(): void {
        ...
        $pages = array_values($DB->get_records('lesson_pages', ['lessonid' => $record->id], 'id ASC'));
        $this->assertStringContainsString('<ul>', $pages[3]->contents);
        $this->assertStringNotContainsString('slide-4.png', $pages[3]->contents);
    }
